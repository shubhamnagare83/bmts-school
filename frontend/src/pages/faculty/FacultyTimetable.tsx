...
export default function FacultyTimetable() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">My Teaching Schedule</h1>
        <p className="text-slate-500 text-sm">Weekly timetable breakdown for your assigned classes</p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle className="text-base font-semibold flex items-center gap-2">
            <Clock className="w-5 h-5 text-indigo-600" /> Weekly Timetable
          </CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
                            ...
                            {slot ? (
                              <div className="space-y-0.5">
                                <div className="font-bold text-indigo-900">
                                  Class {slot.class?.name}-{slot.division?.name}
                                </div>
                                <div className="text-[11px] font-medium text-slate-700">{slot.subject?.name}</div>
